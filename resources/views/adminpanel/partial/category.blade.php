@if(isset($selected_category))
    @foreach($categories as $sub_category)
        ...
        @if(count($sub_category->childrenRecursive) > 0)
            @include('adminpanel.partial.category', ['categories' => $sub_category->childrenRecursive, 'level' => $level+1, 'selected_category'=>$selected_category])
        @endif
    @endforeach
@else
    @foreach($categories as $sub_category)
        ...
        @if(count($sub_category->childrenRecursive) > 0)
            @include('adminpanel.partial.category', ['categories' => $sub_category->childrenRecursive, 'level' => $level+1])
        @endif
    @endforeach
@endif
