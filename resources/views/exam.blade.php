    ...
    <div class="col-sm-9">
        <div class="border mt-5">
            ...
            <div class="container">
                <div class="d-flex flex-wrap flex-sm-row justify-content-around mt-5">
                    <div class="col-sm-5 w-100 center mt-4"><span class="f-1-3">توضیح آزمون:<span>{{$exam->desc}}</span></span>
                    </div>
                    <div class="col-sm-3 100 center mt-4"><span
                            class="f-1-3">تعداد سوالات:<span>{{$exam->question_count}}</span></span></div>
                    <div class="col-sm-3 w-100 center mt-4"><span
                            class="f-1-3">زمان:<span>{{$exam->exam_time}}قیقه</span></span></div>
                </div>
            </div>
            <div class=" mt-5 d-flex justify-content-start">
                <ul class="rtl right">
                    ...
                    <li class="mt-4">در صورت ترک کردن این صفحه بدون ذخیره پاسخ ها و بازگشت دوباره آزمون از اول شروع
                        میشود.
                    </li>
                </ul>
            </div>
            ...
            <div class="right mr-1">
                <form name="myform" action="/answersubmit" method="post" enctype="multipart/form-data">
                    ...
                    @foreach ($questions as $question)
                        <hr>
                        <p class="mainquestion"><span>{{$i+1}}-</span><span>{{$question->question}}</span><span>؟</span>
                        </p><br>
                        @if($question->qhaspic==1)
                            <br>
                            <div class="row justify-content-center">

                                <img class="question-image mt-2"
                                     src="{{ asset('images.questionpic/'.$question->qpicaddress) }}">
                            </div>
                            <br>
                        @endif
                                ...
                                @for($j=0;$j<$exam->option_count;$j++)
                                    @if(isset($options[$i][$j]))
                                        <div style="margin-left:3rem">
                                            <span style="direction:ltr;margin:0 5px" class="f-1-5">{{$j+1}}-</span><span
                                                class="f-1-3" style="margin:0 5px">{{$options[$i][$j]->option}}<span>&nbsp;
        @if($options[$i][$j]->ohaspic==1)
                                                        <img class="option-image"
                                                             src="{{ asset('images.optionpic/'.$options[$i][$j]->opicaddress) }}">
                                                    @endif
        <input class="option-image" type="radio" name="question{{$i}}"
               value="{{$options[$i][$j]->id}}" style="width:3rem;height:3rem;">
                                ...
                                @endfor
                                @endif
                                @if($question->exam->type_question == 1)
                                    <div style="margin-left:3rem">
