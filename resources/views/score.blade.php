...
<div class="container-fluid">
	...
	<div style="border:1px solid #1da1f2;border-radius: 2rem;background-color: white;width: 100%;text-align:center;margin-top: 5rem;direction: rtl">
		...
		<span style="color:#1da1f2 ">جواب درست:</span><span>{{$options[$i][$k]->option}}</span>
		@if($options[$i][$k]->ohaspic==1)
		<img style="margin-right:2rem;max-height: 80px;max-width: 200px;" src="{{ asset('images.optionpic/'.$options[$i][$k]->opicaddress) }}">
		@endif
		@endif
		</p>
		@if(in_array($options[$i][$k]->id, $answers))
		<p style="margin-top: 1rem;"><span style="color:#1da1f2" >جواب شما:</span><span>{{$options[$i][$k]->option}}</span>
		@if($options[$i][$k]->ohaspic==1)
		<img style="margin-right:2rem;max-height: 80px;max-width: 200px;" src="{{ asset('images.optionpic/'.$options[$i][$k]->opicaddress) }}">
		@endif
		@endif
